-- 0086_checkout_terms_gate.sql
-- ADR-191 (H-COMP-01): CHECKOUT NIE SPRZEDAJE BEZ OPUBLIKOWANYCH DOKUMENTÓW
-- PRAWNYCH NAJEMCY — bramka publikacji w app.public_checkout.
--
-- OBJAW (audyt E2E właściciela, 17.08): checkout wymagał checkboxa „Akceptuję
-- regulamin", /regulamin i /prywatnosc oddawały 404, a rezerwacja i tak
-- powstawała. Rejestr wersji istnieje od 0063 (ADR-129), ale gałąź (a)
-- rozstrzygnięcia wersji świadomie przepuszczała najemców bez publikacji
-- („zachowanie sprzed 0063 co do znaku") — storefront wysyłał wtedy stałą
-- „1.0" i zamówienie utrwalało zgodę wskazującą dokument, którego nie ma.
-- Decyzja właściciela z audytu: brak opublikowanych dokumentów = NIE MA
-- sprzedaży, w żadnej powierzchni.
--
-- CO ROBI TA MIGRACJA — dokładnie jedną rzecz:
--   przepisuje app.public_checkout, dokładając bramkę [0086] tuż przed
--   rozstrzygnięciem wersji regulaminu [0063]: najemca bez opublikowanego
--   regulaminu ORAZ polityki prywatności (current_version_id w
--   public.legal_documents) dostaje odmowę 22023 z detail =
--   'legal_documents_missing' (stały komunikat bez identyfikatorów — ADR-181).
--
-- CZEGO NIE ZMIENIA (świadomie):
--   * SYGNATURA BEZ ZMIAN (23 argumenty jak w 0063/0065) — dodanie parametru
--     tworzyłoby PRZECIĄŻENIE (druga funkcja obok starej) i niejednoznaczne
--     wywołania PostgREST w oknie wdrożeniowym. Nie ma drop function, nie ma
--     okna z drugą ścieżką zapisu (wzorzec 0063).
--   * Gałęzie (b)/(c)/(d) rozstrzygnięcia wersji z 0063 — co do znaku.
--     Gałąź (d) (integracja z własną etykietą: API v1, wtyczka WordPress)
--     ŚWIADOMIE zostaje otwarta — to nazwany dług ADR-129, domykany w API v2;
--     zamknięcie go tutaj wywróciłoby wdrożoną integrację w dniu publikacji
--     dokumentów. Ścisłą konfrontację etykiety dla powierzchni, które
--     renderują zgodę z rejestru (storefront, embed), trzyma warstwa rdzenia
--     checkoutu (lib/checkout/core.ts) — patrz ADR-191.
--   * Schemat orders BEZ ZMIAN: kolumna orders.terms_version_id istnieje od
--     0063 (FK złożony (tenant_id, terms_version_id), CHECK
--     orders_terms_version_pinned) i gałąź (b) już ją wypełnia.
--   * Koperty publicznych odczytów BEZ ZMIAN (są parsowane .strict()
--     po stronie sklepu — lekcja 0074/0079/0083).
--
-- OKNO WDROŻENIOWE (migracja jedzie na produkcję PRZED kodem): twarda odmowa
-- w bazie zapala się od chwili wgrania — najemca bez opublikowanych
-- dokumentów zobaczy w oknie ogólny błąd („nie przyjęliśmy zamówienia")
-- zamiast dedykowanego komunikatu, bo stary kod sklepu nie zna jeszcze
-- znacznika. ŚWIADOME DOPUSZCZENIE (decyzja PM w briefie zadania): odmowa
-- jest celem zadania, okno trwa minuty, płacącego najemcy jeszcze nie ma.
-- Mechanizmu przejściowego celowo nie budujemy.
--
-- CIAŁO FUNKCJI: przepisane W CAŁOŚCI z 0065_tenant_commercially_active.sql
-- (NAJNOWSZA definicja — funkcja była przepisywana w 0010/0020/0021/0029/
-- 0040/0041/0049/0058/0059/0063/0065; create or replace zastępuje całe ciało,
-- więc kopia ze starszej wersji cicho cofnęłaby późniejsze poprawki).
-- Różnice wobec 0065 to DOKŁADNIE dwa miejsca, oba oznaczone [0086]:
-- bramka publikacji + komentarz przy (od teraz nieosiągalnej) gałęzi (a).

-- === BEGIN PROD MIGRATION 0086 ===
