...
import { PANEL_BOTTOM_NAV_ITEMS, matchNavItem } from "@/lib/shell/nav";
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      >
        {PANEL_BOTTOM_NAV_ITEMS.map((item) => {
          ...
          const current = active?.id === item.id && !(item.id === "orders" && newOrderActive);
          return (
            <Link
              key={item.id}
              href={item.href}
        ...
        <Link
          ...
          className={`${BOTTOM_ITEM_CLASS} bg-accent text-accent-foreground m-1 min-h-14 rounded-md px-0 py-1`}
        >
