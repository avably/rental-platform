...
/**
 * PŁÓTNO KREATORA (K1, ADR-083) — szkic strony renderowany TYM SAMYM
 * `SiteRenderer` co sklep, z warstwą edycyjną wniesioną przez `sectionWrapper`.
 *
 ...
 * narzędzi i miejscami na „+". Fork renderera dałby dwa źródła prawdy o
 * wyglądzie sekcji i każdy nowy typ trzeba by pisać dwa razy; warstwa edycyjna
 * wchodzi więc SZWEM, a nie kopią (kontrakt: apps/storefront nie podaje owijki,
 ...
 * niesie tu żaden filtr UI, tylko `app.get_published_site` (0019), który
 * oddaje anonowi wyłącznie sekcje `enabled` z `content_published`.
 *
 * Kolejność zmienia się DWOMA drogami wołającymi TĘ SAMĄ akcję z pełnym
 * kompletem pozycji: przeciągnięciem za uchwyt (dnd-kit, aktywacja tylko z
 * uchwytu — reszta płótna to treść strony) i strzałkami w pasku narzędzi
 * (fallback klawiaturowy). Zmiana jest optymistyczna, błąd COFA układ.
 */
import {
  DndContext,
  KeyboardSensor,
        ...
        {order.length === 0 ? (
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={orderedIds} strategy={verticalListSortingStrategy}>
              <SiteRenderer
                sections={order as unknown as RenderSection[]}
                template={template}
                products={products}
  ...
  return (
    ...
    >
      ...
      {/* Obrys sekcji: warstwa NAD treścią, ale przepuszczająca kliknięcia —
          treść płótna nie jest jeszcze klikalna (edycja w miejscu to K2/K3),
