  ...
  if (!enrollState.factorId) {
    return (
      <form action={enrollFormAction} className="flex flex-col gap-3">
        ...
        >
          {enrollPending ? "Przygotowuję…" : "Włącz uwierzytelnianie dwuskładnikowe"}
        </button>
      </form>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-sm text-gray-600">
        Zeskanuj kod QR aplikacją uwierzytelniającą (np. Google Authenticator) albo wpisz sekret
        ręcznie: <code className="break-all">{enrollState.secret}</code>
      </p>
      {enrollState.qrCode ? (
        // eslint-disable-next-line @next/next/no-img-element -- Supabase zwraca gotowy data:image/svg+xml, bez optymalizacji next/image.
        <img src={enrollState.qrCode} alt="Kod QR TOTP" width={200} height={200} />
      ) : null}
      <form action={verifyFormAction} className="flex flex-col gap-3">
        ...
        <button
          ...
          className="rounded bg-black px-3 py-2 text-white disabled:opacity-50"
        >
          {verifyPending ? "Weryfikuję…" : "Potwierdź kod"}
        </button>
      </form>
    </div>
  );
