...
const approvedLogoAPath = "M100.8 69.8588L116.599 20.2656H130.556L146.355 69.8588H138.774L135.303 58.8775H111.781L108.31 69.8588H100.8ZM113.907 52.0053H133.177L123.542 21.3992L113.907 52.0053Z";
...
const assertDotOnlyColorSymbol = (source, variant) => {
  assert.match(source, /<svg\b[^>]*viewBox="0 0 96 96"/);
  assert.match(source, /<rect[^>]*width="96"[^>]*height="96"[^>]*rx="25"[^>]*fill="#0B1017"/);
  assert.match(source, /<circle[^>]*data-brand-dot[^>]*cx="48"[^>]*cy="48"[^>]*r="25"[^>]*fill="#A8C743"/);
  assert.doesNotMatch(source, /<path\b|transform=/, `${variant} nie może zawierać litery ani przesuniętego znaku`);
};

assertDotOnlyColorSymbol(svgSources.get("symbol-color"), "symbol-color");
for (const variant of ["symbol-mono", "symbol-negative"]) {
  const source = svgSources.get(variant);
  assert.match(source, /<svg\b[^>]*viewBox="0 0 96 96"/);
  assert.match(source, /<path[^>]*fill-rule="evenodd"[^>]*d="M0 25Q0 0 25 0H71Q96 0 96 25V71Q96 96 71 96H25Q0 96 0 71ZM48 23A25 25 0 1 0 48 73A25 25 0 1 0 48 23Z"/);
  assert.doesNotMatch(source, new RegExp(approvedLogoAPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  assert.doesNotMatch(source, /<circle\b|transform=/);
}
for (const variant of [
  ...
  assertDotOnlyColorSymbol(svgSources.get(variant), variant);
}
